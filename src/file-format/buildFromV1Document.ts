...
//   Vec2dModel,
//   createShapeId,
// } from "../tlschema/records/TLShape";
import { Vec2d } from "../primitives/Vec2d";
import { clamp } from "../primitives/utils";
import { Vec2dModel } from "../tlschema/misc/geometry-types";
import {
  AssetRecordType,
  TLAsset,
  TLAssetId,
} from "../tlschema/records/TLAsset";
import { PageRecordType, TLPageId } from "../tlschema/records/TLPage";
import {
  TLShapeId,
  TLShapePartial,
  createShapeId,
} from "../tlschema/records/TLShape";
import { TLArrowShape, TLArrowTerminal } from "../tlschema/shapes/TLArrowShape";
import { TLDrawShape } from "../tlschema/shapes/TLDrawShape";
import { TLGeoShape } from "../tlschema/shapes/TLGeoShape";
import { TLImageShape } from "../tlschema/shapes/TLImageShape";
import { TLNoteShape } from "../tlschema/shapes/TLNoteShape";
import { TLTextShape } from "../tlschema/shapes/TLTextShape";
import { TLVideoShape } from "../tlschema/shapes/TLVideoShape";
import { TLAlignType } from "../tlschema/styles/TLAlignStyle";
import { TLArrowheadType } from "../tlschema/styles/TLArrowheadStyle";
import { TLColorType } from "../tlschema/styles/TLColorStyle";
import { TLDashType } from "../tlschema/styles/TLDashStyle";
import { TLFontType } from "../tlschema/styles/TLFontStyle";
import { TLSizeType } from "../tlschema/styles/TLSizeStyle";

const TLDRAW_V1_VERSION = 15.5;

/** @internal */
export function buildFromV1Document(
  editor: Editor,
    ...
    Object.values(document.pages ?? {})
      ...
      .forEach((v1Page) => {
        ...
        function decideNotToCreateShape(v1Shape: TDShape) {
          ...
          const v1GroupParent = v1GroupShapeIdsToV1ChildIds.has(
            v1Shape.parentId
          );
          if (v1GroupParent) {
        ...
        v1Shapes.forEach((v1Shape) => {
          ...
          switch (v1Shape.type) {
            ...
            case TDShapeType.Rectangle: {
              ...
              const pageBoundsBeforeLabel = editor.getPageBoundsById(
                inCommon.id
              )!;
              ...
              if (
                pageBoundsBeforeLabel.width === pageBoundsBeforeLabel.height
              ) {
        ...
        v1GroupShapeIdsToV1ChildIds.forEach((v1ChildIds, v1GroupId) => {
          const v2ChildShapeIds = v1ChildIds.map(
            (id) => v1ShapeIdsToV2ShapeIds.get(id)!
          );
          ...
          for (const handleId of ["start", "end"] as const) {
            ...
            if (bindingId) {
              ...
              if (targetId) {
                ...
                const v2ShapeFresh =
                  editor.getShapeById<TLArrowShape>(v2ShapeId)!;

                const nx = clamp(
                  (coerceNumber(binding.point[0]) + 0.5) / 2,
                  0.2,
                  0.8
                );
                const ny = clamp(
                  (coerceNumber(binding.point[1]) + 0.5) / 2,
                  0.2,
                  0.8
                );
    ...
    const file = new File(
      ...
      placeholderAsset.props.name,
      {
        type:
          ...
          placeholderAsset.props.mimeType ??
          undefined,
      }
    );
