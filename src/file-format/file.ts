import { TLStore } from "../tlschema/TLStore";
import { TLAsset, TLAssetId } from "../tlschema/records/TLAsset";
import { TLInstanceId } from "../tlschema/records/TLInstance";
import { TLRecord } from "../tlschema/records/TLRecord";
import { RecordId, UnknownRecord } from "../tlstore/base-record";
import { MigrationFailureReason, MigrationResult } from "../tlstore/migrate";
import { StoreSnapshot } from "../tlstore/store";
import { SerializedSchema } from "../tlstore/store-schema";
import { Result, exhaustiveSwitchError } from "../tlutils/control";
...
export function parseTldrawJsonFile({
  ...
  try {
    const storeSnapshot = Object.fromEntries(
      data.records.map((r) => [r.id, r as TLRecord])
    );
    migrationResult = store.schema.migrateStoreSnapshot(
      storeSnapshot,
      data.schema
    );
  } catch (e) {
...
export async function serializeTldrawJson(store: TLStore): Promise<string> {
      ...
      case "asset":
        if (
          ...
          try {
            ...
            assetSrcToSave = await fileToBase64(
  ...
  const recordsToSave = records.concat(
    assets.filter((a) => usedAssets.has(a.id))
  );
...
export async function serializeTldrawJsonBlob(store: TLStore): Promise<Blob> {
  return new Blob([await serializeTldrawJson(store)], {
...
export async function parseAndLoadDocument(
  ...
  editor.replaceStoreContentsWithRecordsForOtherDocument(
