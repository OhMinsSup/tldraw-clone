...
export class Vec2d {
  ...
  static Snap(A: VecLike, step = 1) {
    return new Vec2d(
      Math.round(A.x / step) * step,
      Math.round(A.y / step) * step
    );
  }
  ...
  static Clamp(A: Vec2d, min: number, max?: number) {
    if (max === undefined) {
      return new Vec2d(
        Math.min(Math.max(A.x, min)),
        Math.min(Math.max(A.y, min))
      );
    }
  ...
  static SnapToGrid(A: VecLike, gridSize = 8) {
    return new Vec2d(
      Math.round(A.x / gridSize) * gridSize,
      Math.round(A.y / gridSize) * gridSize
    );
  }
}
