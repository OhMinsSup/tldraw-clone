...
export function getPolygonVertices(
  ...
  for (let i = 0; i < sides; i++) {
    ...
    pointsOnPerimeter.push(
      new Vec2d(cx + cx * Math.cos(t), cy + cy * Math.sin(t))
    );
  }
 ...
 * @public
 */
export function pointNearToPolyline(
  A: VecLike,
  points: VecLike[],
  distance = 8
) {
 ...
 * @public
 */
export function pointNearToLineSegment(
  A: VecLike,
  p1: VecLike,
  p2: VecLike,
  distance = 8
...
export function simplify(points: VecLike[], tolerance = 1): VecLike[] {
  ...
  if (len > 2) {
    ...
    for (let i = 1; i < len - 1; i++) {
      ...
      const d =
        Math.pow(x0 * (y2 - y1) + x1 * (y0 - y2) + x2 * (y1 - y0), 2) / max;
