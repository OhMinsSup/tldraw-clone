...
import { Migrations, defineMigrations } from "../tlstore/migrate";
import { createRecordType } from "../tlstore/record-type";
import { StoreSchema } from "../tlstore/store-schema";
...
export function createTLSchema(
  ...
  const allShapeEntries = Object.entries({
    ...coreShapes,
    ...defaultShapes,
    ...customShapes,
  });
  ...
  const ShapeRecordType = createRecordType<TLShape>("shape", {
    migrations: defineMigrations({
      ...
      subTypeMigrations: {
        ...Object.fromEntries(
          allShapeEntries.map(([k, v]) => [
            k,
            v.migrations ?? defineMigrations({}),
          ])
        ),
      },
    }),
    ...
    validator: T.model(
      ...
      T.union("type", {
        ...Object.fromEntries(
          allShapeEntries.map(([k, v]) => [
            k,
            (v.validator as T.Validator<any>) ?? T.any,
          ])
        ),
      })
    ),
