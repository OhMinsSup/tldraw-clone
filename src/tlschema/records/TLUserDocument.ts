...
import { TLInstance, instanceIdValidator } from "./TLInstance";
import { TLPage, pageIdValidator } from "./TLPage";
 ...
 * @public
 */
export interface TLUserDocument
  extends BaseRecord<"user_document", TLUserDocumentId> {
  isPenMode: boolean;
  isGridMode: boolean;
  ...
  migrators: {
    ...
    [Versions.AddMissingIsMobileMode]: {
      up: (userDocument: TLUserDocument) => {
        return {
          ...userDocument,
          isMobileMode: userDocument.isMobileMode ?? false,
        };
      },
    ...
    [Versions.RemoveIsReadOnly]: {
      up: ({
        isReadOnly: _,
        ...userDocument
      }: TLUserDocument & { isReadOnly: boolean }) => {
        return userDocument;
      },
...
/** @internal */
export const UserDocumentRecordType = createRecordType<TLUserDocument>(
  "user_document",
  {
