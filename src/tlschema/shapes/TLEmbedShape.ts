...
export const EMBED_DEFINITIONS = [
  ...
  },
  {
    ...
    fromEmbedUrl: (url) => {
      ...
      if (
        matches &&
        urlObj.searchParams.has("center") &&
        urlObj.searchParams.get("zoom")
      ) {
  ...
  },
  {
    ...
    toEmbedUrl: (url) => {
      ...
      if (
        urlObj?.pathname.match(/^\/presentation/) &&
        urlObj?.pathname.match(/\/pub\/?$/)
      ) {
      ...
      overridePermissions: T.dict(
        T.setEnum(
          new Set(
            ...
            ) as (keyof TLEmbedShapePermissions)[]
          )
        ),
        T.boolean.optional()
      ).optional(),
