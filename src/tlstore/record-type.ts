...
> {
  ...
  parseId(id: string): IdOf<R> {
    if (!this.isId(id)) {
      throw new Error(
        `ID "${id}" is not a valid ID for type "${this.typeName}"`
      );
    }
  ...
  withDefaultProperties<
    DefaultProps extends Omit<Partial<R>, "typeName" | "id">
  >(
    createDefaultProperties: () => DefaultProps
  ): RecordType<R, Exclude<RequiredProperties, keyof DefaultProps>> {
    return new RecordType<R, Exclude<RequiredProperties, keyof DefaultProps>>(
      this.typeName,
      {
...
export function assertIdType<R extends UnknownRecord>(
  ...
  if (!id || !type.isId(id)) {
    throw new Error(
      `string ${JSON.stringify(id)} is not a valid ${type.typeName} id`
    );
  }
}
