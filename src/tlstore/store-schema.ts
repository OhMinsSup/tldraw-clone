...
export class StoreSchema<R extends UnknownRecord, P = unknown> {
  static create<R extends UnknownRecord, P = unknown>(
    ...
    // @ts-expect-error - types are correct, TS just can't infer them
    return new StoreSchema<R, P>(types, options ?? {});
  }
  ...
  validateRecord(
    ...
    try {
      ...
      if (!recordType) {
        throw new Error(
          `Missing definition for record type ${record.typeName}`
        );
      }
          ...
          type.migrations.subTypeKey && type.migrations.subTypeMigrations
            ? {
                ...
                subTypeVersions: type.migrations.subTypeMigrations
                  ? Object.fromEntries(
                      Object.entries(type.migrations.subTypeMigrations).map(
                        ([k, v]) => [k, v.currentVersion]
                      )
                    )
                  : undefined,
              }
