import { exhaustiveSwitchError } from "../tlutils/control";
import { getOwnProperty, hasOwnProperty } from "../tlutils/object";
...
/** @public */
export type TypeOf<V extends Validator<unknown>> = V extends Validator<infer T>
  ? T
  : never;
...
export class ObjectValidator<Shape extends object> extends Validator<Shape> {
  constructor(
    ...
    super((object) => {
      if (typeof object !== "object" || object === null) {
        throw new ValidationError(
          `Expected object, got ${typeToString(object)}`
        );
      }

      for (const [key, validator] of Object.entries(config)) {
        prefixError(key, () => {
          (validator as Validator<unknown>).validate(
            getOwnProperty(object, key)
          );
        });
      }
  ...
  extend<Extension extends Record<string, unknown>>(extension: {
    ...
    return new ObjectValidator({
      ...this.config,
      ...extension,
    }) as ObjectValidator<Shape & Extension>;
  }
}
...
> extends Validator<TypeOf<Config[keyof Config]> | UnknownValue> {
  constructor(
    ...
    super((input) => {
      ...
      const matchingSchema = hasOwnProperty(config, variant)
        ? config[variant]
        : undefined;
...
export const nonZeroNumber = number.check((value) => {
  if (value <= 0)
    throw new ValidationError(
      `Expected a non-zero positive number, got ${value}`
    );
});
/**
...
export const nonZeroInteger = integer.check((value) => {
  if (value <= 0)
    throw new ValidationError(
      `Expected a non-zero positive integer, got ${value}`
    );
});
...
export function setEnum<T>(values: ReadonlySet<T>): Validator<T> {
  return new Validator((value) => {
    if (!values.has(value as T)) {
      const valuesString = Array.from(values, (value) =>
